...
Scanner::Scanner(std::string program_text) :
		m_text(std::move(program_text)),
		m_idSymTable(new SymbolTable()),
		m_constantsSymTable(new SymbolTable()),
		m_finiteAutomata(new FiniteAutomata("../fa.in")) {
	parse();
}
...
//	std::regex constants_regex(CONSTANTS_REGEX);
	...
	for (const std::pair<int, std::string>& lexPair: lexemes) {
		...
		std::string lex_type = m_finiteAutomata->verifyLexeme(lex);
		if (reserved_tokens.find(lex) != reserved_tokens.end()) {
			pif << "[-1, " << lex << " ]" << "\n";
		} else if (lex_type == "IDENTIFIER") {
			// add to id symtable
		...
		} else if (lex_type == "INTEGER_CONSTANT") {
