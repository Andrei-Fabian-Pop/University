...
class StudentService:
    ...
    def add_student(self, _id, name, history_service):
        """
        ...
        """
        try:
            for item in self.__stud_repo.get_all():
                if int(_id) == int(item.id):
                    raise ValueError
            self.__stud_repo.add(Student(int(_id), name))
            history_service.add_expression("add", "student", [_id, name])
        except ValueError:
