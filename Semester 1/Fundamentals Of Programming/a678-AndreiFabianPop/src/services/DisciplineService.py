...
class DisciplineService:
    ...
    def add_discipline(self, name, history_service):
        """
        ...
        """
        if self.__id_dump:
            self.__discipline_repo.add(Discipline(self.__id_dump[-1], name))
            history_service.add_expression("add", "discipline", [self.__id_dump[-1], name])
            self.__id_dump.pop()
        else:
            self.__discipline_repo.add(Discipline(self.__id, name))
            history_service.add_expression("add", "discipline", [self.__id, name])
            self.__id += 1
