...
void Admin::add() {
    ...
    int result = -1;
    try {
        result = service.add(title, presenter, duration_minutes, duration_seconds, likes, link);
    } catch (TutorialException &exceptions) {
        for (auto &exception: exceptions.getErrors()) {
            std::cerr << exception;
        }
    } catch (DuplicateException &duplicateException) {
        std::cerr << duplicateException.what();
    }

    if (result == 1) {
        std::cout << "Tutorial added successfully.\n";
    } else if (result == -1) {
        std::cout << "Tutorial was not added due to errors.\n";
    }
}
...
void Admin::remove() {
    ...
    try {
        ...
        std::cerr << incorrectPosition.what();
    }
}
    ...
    while (client_command != 0) {
        ...
        if (client_command < 0 || client_command > MAX_OPTION) {
            ...
            if (client_command == 1) {
                ...
                print_watchlist();
            } else if (client_command == 4) {
                save();
            } else if (client_command == 5) {
                open();
            }
        }
    }
...
void Client::print_by_presenter() {
    ...
    std::vector<Tutorial> arr = service.get_by_presenter(presenter)->getAll();
    WatchList<Tutorial> *watchList = service.get_by_presenter(presenter);
    int count = 0;
    std::for_each(arr.begin(), arr.end(), [&](const Tutorial &tutorial) {
        std::cout << count++ << ". " << tutorial.get_title() << ", by " << tutorial.get_presenter() <<
                  " with a length of " << tutorial.get_duration_minutes() << ":" << tutorial.get_duration_seconds() <<
    ...
    while (!arr.empty()) {
        ...
        std::cout << "Tutorial options:\n"
                     ...
                     "3. Next\n"
                     "0. Stop\n";
        Tutorial tutorial = watchList->getCurrent();
        client_option = get_user_input();
        if (client_option == 1) {
            this->service.add_to_playlist(tutorial);
            watchList->remove(tutorial);
        } else if (client_option == 2) {
            watchList->next();
        } else if (client_option == 3) {
            this->service.like(this->service.get_index(tutorial.get_title(), tutorial.get_presenter(), tutorial.get_link()));
...
void Client::print_watchlist() {
    std::vector<Tutorial> watchlist = service.get_watchList();
    int count = 0;
