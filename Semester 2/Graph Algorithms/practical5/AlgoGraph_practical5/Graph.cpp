...
void Graph::DFS(int currentNode, bool *visited, std::list<int> &component) {
    ...
    for (int &it: this->arcsMap[currentNode]) {
        if (!visited[it]) {
...
std::list<std::list<int>> Graph::connectedComponents() {
    ...
    for (const auto &graph: compList) {
        ...
        for (auto vertex: graph) {
            for (auto connection = this->arcList(vertex).first;
                 connection != this->arcList(vertex).second; ++connection) {
                try {
...
int Graph::findMaxClique(int i=0, int l=1) {
    std::vector<int> store(this->nrNodes());
    int _max = 0, n = this->nrNodes();

    for (int j = i +1; j <= n; ++j) {
        store[l] = j;

        if (this->is_clique(l + 1)) {
            _max = std::max(_max, l);
            _max = std::max(_max, findMaxClique(j, l+1));
        }
    }
    return _max;
}
